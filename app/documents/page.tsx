            ...
            <Link key={doc.id} href={`/documents/${doc.id}`}
              ...
              <div className="flex items-start gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">{doc.category} · v{doc.version} · updated {doc.updatedAt}</p>
                </div>
              ...
              <div className="flex items-center gap-3 text-xs">
                {doc.findingsCount > 0 && (
                  <span className="flex items-center gap-1 text-orange-600 font-medium">
                    <AlertTriangle size={12} /> {doc.findingsCount} finding{doc.findingsCount !== 1 ? "s" : ""}
                  </span>
                )}
                {doc.usesSupersededReference && (
                  <span className="flex items-center gap-1 text-red-600 font-medium">
                    <Layers size={12} /> uses superseded reference
                  </span>
                )}
                {doc.findingsCount === 0 && !doc.usesSupersededReference && (
                  <span className="text-gray-400">No open findings</span>
                )}
              </div>
            </Link>
          );
...
function Templates() {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-gray-500 text-sm">{MOCK_TEMPLATES.length} templates · none are pre-approved or certified as court-compliant</p>
        <Link href="/documents/draft" className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors">
          ...
          <div key={tpl.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-shadow">
            <div className="flex items-start gap-3 mb-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400">{tpl.category}{tpl.builtIn ? " · built-in" : " · custom"}</p>
              </div>
            </div>
            <p className="text-sm text-gray-500 mb-3">{tpl.description}</p>
            <div className="flex flex-wrap gap-1.5 mb-3">
              {tpl.variables.slice(0, 4).map((v) => (
                <span key={v} className="text-[10px] font-mono bg-gray-50 border border-gray-200 text-gray-500 px-1.5 py-0.5 rounded">{v}</span>
              ))}
              {tpl.variables.length > 4 && <span className="text-[10px] text-gray-400">+{tpl.variables.length - 4} more</span>}
            </div>
